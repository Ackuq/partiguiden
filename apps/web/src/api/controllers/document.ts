...
import type { MemberDocuments } from "../../types/member";
...
export const getJsonDocument = (
  id: string,
): Promise<DocumentStatus<UnknownDocumentStatus>> =>
...
export const searchDocument = (query: ParsedUrlQuery): Promise<DocumentList> =>
  fetch(`${parliamentURL}/dokumentlista/?${stringify(query)}`).then((res) =>
    res.json(),
  );
...
export const jsonDocumentController = async (
  id: string,
...
export const memberDocumentsController = (
  id: string,
  page?: string,
): Promise<MemberDocuments> => {
