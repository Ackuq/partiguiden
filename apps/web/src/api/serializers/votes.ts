...
import type { VoteList, VoteListEntry } from "../../types/voting";
import { getVoteResult } from "../controllers/vote";
import { titleTrim } from "../helpers/voteUtils";

export const voteSerializer = async (
  data: DocumentListEntry,
): Promise<VoteListEntry> => {
...
export const votesSerializer = (data: DocumentList): Promise<VoteList> => {
  ...
  const votesPromises: Array<Promise<VoteListEntry>> =
    unserializedVotes.map(voteSerializer);
