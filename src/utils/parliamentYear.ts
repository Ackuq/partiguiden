import { ParliamentYearResponse, ParliamentYearsResponse } from '../types/parliament';

const fetchParliamentYears = (): Promise<ParliamentYearsResponse> => {
  return fetch('http://data.riksdagen.se/sv/koder/?typ=riksmote&utformat=json').then((res) =>
    res.json()
  );
};
...
export const getLatestParliamentYear = async (): Promise<string> => {
  ...
  const latestParliamentYear = parliamentYears.riksmoten.riksmote.pop() as ParliamentYearResponse;
...
export const getLatestMandatePeriod = async (): Promise<MandatePeriod> => {
  ...
  let parliamentYear = parliamentYears.riksmoten.riksmote.pop() as ParliamentYearResponse;
  while (parliamentYear.mandatperiod === mandatePeriod.period) {
    mandatePeriod.parliamentYears.push(parliamentYear.riksmote);
    parliamentYear = parliamentYears.riksmoten.riksmote.pop() as ParliamentYearResponse;
  }
